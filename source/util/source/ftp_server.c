...
#define debug_retval(ret_val)                                                  \
  do {                                                                         \
    LOG_INFO("Line %d: Return value: %d, errno: %d (\"%s\").", __LINE__,    \
...
#define debug_func(ret_val)                                                    \
  do {                                                                         \
    if (ret_val) {                                                             \
      LOG_INFO("Line %d: " #ret_val ": Return value: %d, errno:"              \
                " %d (\"%s\").",                                             \
...
#define SELF_ORBIS_MAGIC        0x1D3D154F
 ...
 bool Check_ELF_Magic(const char* path, uint32_t FILE_MAGIC) {
  ...
  if (file_fd < 0) {
    LOG_INFO("Error opening file: %s", path);
    return false;
  }
    ...
    size_t bufsize)
{
    ...
    if(strstr(file_path, "safemode.elf") != NULL){
      LOG_INFO("safemode.elf CANNOT be decrypted");
      return -1;
    }
    ...
    size_t bufsize)
{
    ...
    if (fd_in < 0) {
        LOG_INFO("Failed to open input file %s", file_path);
        return -1;
    }
    ...
    if (rnps_decrypt_block(file_buf, (unsigned int)bytes_read) != 0) {
      //  LOG_INFO("Failed DECRYPTION OF %S", file_buf);
        result = -4;
        goto cleanup;
    }
    ...
    if (fd_out < 0) {
        LOG_INFO("Failed to open output file %s, %s", temp_path, strerror(errno));
        result = -5;
        goto cleanup;
    }
      temp_created = true;
    ...
    if (bytes_written < 0 || bytes_written != bytes_read) {
        LOG_INFO("Failed to write to output file %s, %s", temp_path, strerror(errno));
        result = -6;
        goto cleanup;
    }
...
static int gen_ftp_path(char *buf, size_t buf_size, struct client_info *client,
  ...
  if (n >= 0 && (unsigned int)n + 1 > buf_size) { // FTP path got truncated.
    LOG_INFO("Generated path larger than buffer.");
    return -1;
  } else {
    return 0;
  }
...
static void *client_thread(void *arg) {
  ...
  LOG_INFO("Client %s connects to socket %d.", client->ipv4,
          client->ctrl_sockfd);
  ...
  while (1) {
    ...
    if (n > 0) {
      ...
      if (cmd_end) {
        *cmd_end = '\0';
        ...
        if (n == sizeof(client->cmd_line) - 1) {
          LOG_INFO("Received command line too long.");
        } else {
          LOG_INFO("Received command line not terminated.");
        }
      ...
      if (n != (int)SCE_NET_ERROR_EINTR) { // Would happen on fini().
        LOG_INFO("Error %d in client %s@%d's thread.", n, client->ipv4,
                  client->ctrl_sockfd);
      }
#else
      LOG_INFO("Error in client %s@%d's thread: %s.", client->ipv4,
  ...
  LOG_INFO("Client %s@%d disconnects.", client->ipv4, client->ctrl_sockfd);

  // Clean up.
  client_list_delete(client);
  debug_func(SOCKETCLOSE(client->ctrl_sockfd));
  close_data_connection(client);
  LOG_INFO("Client %s@%d's thread exits.", client->ipv4,
            client->ctrl_sockfd);
  free(client);
  return NULL;
}
...
static void *server_thread(void *arg) {
  ...
  // Create the FTP command list (once; never freed - process lifetime).
  static bool command_list_ready = false;
  if (!command_list_ready) {
    if (create_command_list()) {
      run = 0;
      return NULL;
    }
    command_list_ready = true;
  }
  ...
  int ret = sceNetSetsockopt(server_sockfd, SOL_SOCKET, SO_REUSEADDR, &option_value,
             sizeof(option_value));
  LOG_INFO("sceNetSetsockopt: %d", ret);
  ...
  while (1) {
    ...
    if (client_sockfd < 0) {
      ...
      if (pthread_create(&client->thid, NULL, client_thread, client)) {
        LOG_INFO("Could not create a client thread.");
        free(client);
        debug_func(SOCKETCLOSE(client_sockfd));
      }
    }
...
int init(const char *ip, unsigned short port, const char *default_directory) {
  ...
  if (inet_pton(AF_INET, ip, &server_ip) == 0) {
    LOG_INFO("Invalid IPv4 address: \"%s\"", ip);
    return -1;
  }
  ...
  if (ret) {
    LOG_INFO("Could not create the client list mutex (error %d).", ret);
    return -1;
  }

  // Create server thread (detached: shutdown never joins from the IPC path).
  ret = pthread_create(&server_thid, NULL, server_thread,
                       (void *)default_directory);
  if (ret) {
    LOG_ERROR("Could not create the server thread (error %d).", ret);
...
void fini(void) {
  // Exit server thread (detached — never join; socket abort unblocks accept).
  debug_func(sceNetSocketAbort(server_sockfd, 0));
  LOG_INFO("Waiting for client threads to exit...");
...
bool StartFTP(void) {
  ...
  if (get_ip_address(ftp_ip, sizeof(ftp_ip)) < 0){
    LOG_ERROR("[FTP] Failed to get IP address");
    onion_notify(true, "notify.ftp.failed", "no IP address");
    return false;
  }
...
void check_ftp_addr_change(void) {
  ...
  char func_ip_address[ONION_NET_IP_ADDRESS_SIZE] = {};
  if (get_ip_address(func_ip_address, sizeof(func_ip_address)) < 0)
    return;
...
/* OnionHEN service wrapper: idempotent, fully non-blocking. */
bool ftp_server_running(void) { return ftp_started; }

void ftp_server_apply(bool enabled) {
  if (enabled) {
    if (!ftp_started)
      (void)StartFTP();
  } else if (ftp_started) {
