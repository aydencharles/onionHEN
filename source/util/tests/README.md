...
| `test_builtin_services` | built-in pkgnet service port allocation |
...
| `test_toolbox_route` | resource → page routing + dynamic cheat enabled-state, Kstuff and PkgNet descriptors |
| `test_toolbox_i18n` | locale-key parity for toolbox groups (zh-Hans, en) including pkgnet keys |
| `test_toolbox_pkg_xml` | PkgInstaller chooser XML, PkgNet toggle XML, resource identifiers |
